...
| **Контракт Vesting** | Готов, **per-user clock**, 36 мес линейно, есть `addInvestor*` для OTC, `activateMerkle` принимает wei |
...
- **~162 460.73 ANGT + OTC резерв** → `Vesting` контракт (когда открываешь vesting-клейм)
...
- [ ] **Тесты** на `MerkleAirdrop`, `Vesting`, `AllocationRegistry`
...
| **Vesting** | [contracts/PresaleVesting.sol](contracts/PresaleVesting.sol) | Vesting для покупателей пресейла + ручное добавление OTC-инвесторов. Per-user clock, 36 мес линейно |
...
Все трое (`FlyANGT`, `MerkleAirdrop`, `Vesting`) под управлением одного Safe-кошелька `0xBBC7Ee82284416aaA9C3e6d9C73d7D1f7752490A`.
...
ignition/modules/PresaleVesting.ts   → useModule(FlyANGT) → Vesting(token, root, owner)
...
7. **С Safe-кошелька** (через Safe Tx Builder в браузере):
   ...
   - `Vesting.setMerkleRoot(rootVesting)`
   - `FlyANGT.transfer(MerkleAirdrop, sumAirdrop)`
   - `FlyANGT.transfer(Vesting, sumVesting + reserveOTC)`
   - `MerkleAirdrop.start()`
   - `Vesting.start()`
